...
\node [block,below of=step1,node distance=1cm] (step2) {\footnotesize Calculate $Z$ and $U^\mathrm{dep}$ for all $NVT$ simulations};
...
\node [block,below of=stepUdep,node distance=1cm] (step5a) {\footnotesize Calculate $A^\mathrm{dep}_\mathrm{liq}$ (Eq.~\ref{eqn:aDepCorrection}) based on $T^\mathrm{sat}_\mathrm{new}$};
\node [block,below of=step5a,node distance=1cm] (step6) {\footnotesize Calculate $\rho_\mathrm{vap}^\mathrm{new}$= RHS of Eq.~\ref{eqn:rhoV} evaluated with $\rho_\mathrm{vap}^\mathrm{old}$};
\node [block,below of=step6,node distance=1cm] (step6b) {\footnotesize Calculate $P^\mathrm{sat}$ (Eq.~\ref{eqn:psat})};
\node [decision,below of=step6b,node distance=2.2cm] (step8) {\footnotesize $|\rho_\mathrm{vap}^{new}-\rho_\mathrm{vap}^{old}|<tol$ ?};
...
\node [smallblock,left of=step8,node distance=4.5cm] (step7) {\footnotesize Update $Z_\mathrm{liq}$ (Eq.~\ref{eqn:zliq}) Set $\rho_\mathrm{vap}^\mathrm{old}=\rho_\mathrm{vap}^\mathrm{new}$};

\path [line] (pre1) -- (step1);
\path [line] (step1) -- (step2);
\path [line] (step2) -- (stepstep);
\path [line] (stepstep) -- (step5);
\path [line] (step5) -- node {\footnotesize Set $Z_\mathrm{liq}=0.0$, $\rho_\mathrm{vap}^\mathrm{old}=0.0$}(step4);
