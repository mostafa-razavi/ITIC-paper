...
%with the desirable $\frac{Z-1}{\rho}$ format from which we subtract Eq.~(\ref{eqn:path2-ig}-\ref{eqn:path4-ig}) to cancel out the effect of adding them. Eq.~(\ref{eqn:5Integrals=Zvap-Zliq}) provides the result of these additions
...
Calculate $U^\mathrm{dep}_\mathrm{liq,new}$ from $U^\mathrm{dep}$ vs. $1/T$, i.e., $U^\mathrm{dep}(T^\mathrm{sat}_\mathrm{new})$ };
