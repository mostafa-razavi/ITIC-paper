...
\begin{figure}
\includegraphics[scale=0.55]{Figures/ITIC-pathway-C2-3.png}
\caption{A schematic plot of the pathways taken in the ITIC method. Squares and diamonds represent non-saturated state points obtained from simulation, while blue filles circles represent estimated $T^\mathrm{sat}$ values. Green and purple diamonds show the state points required for $B_2$ calculation at isothermal temperature and $T_\mathrm{r}=0.9$, respectively. Note that circle points 11, 13, 15, 17, and 19 represent the initial estimate of saturation temperature, therefore they do not necessarily match the coexistence curve.}
